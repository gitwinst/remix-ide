...
1. Native plugins that are always loaded** (so you don't need to activate them in the url parameter **activate**). These include: **fileManager**, **settings**, **manager** (the plugin manager), and **udapp** (deploy & run).
2. Native plugins that are loaded on demand: **debugger**, **hardhat-provider**, **solidity**, **solidityStaticAnalysis**, **solidityUnitTesting**, and **vyper** 
...
```
https://remix.ethereum.org/?#activate=defiexplorer,solidity&call=fileManager//open//3_Ballot.sol
```
